...
export function createClient(input?: ConfigInput) {
    return new VictoriaLogsClient(input);
}
 ...
 * @param input
 */
export function isClient(input: unknown): input is VictoriaLogsClient {
    if (input instanceof VictoriaLogsClient) {
        return true;
    }

    return hasInstanceof(input, VICTORIALOGS_CLIENT_INSTANCE);
}
